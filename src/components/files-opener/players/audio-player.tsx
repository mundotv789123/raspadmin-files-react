...
export default function AudioPlayer(props: { filesEvent: EventEmitter, filesList?: Array<FileDTO> }) {
  ...
  const audioTimePercent = useMemo(() => audioProps.currentTime / audioProps.duration, [audioProps.duration, audioProps.currentTime]);
  const audioPlayList = useMemo(() => (audioControls.random && playlist) ? playlist.map(a => a).sort(() => Math.random() - 0.5) : playlist, [playlist, audioControls.random]);
  ...
  function handlerAudioLoaded() {
    ...
    navigator.mediaSession.metadata.title = audioControls.hideTitle ? "Raspadmin Music Player" : file!.name;
    navigator.mediaSession.metadata.artwork = [{ src: (!file!.icon || audioControls.hideTitle) ? "/img/icons/music.svg" : file!.icon }];
  ...
  function handlerError() {
    ...
    setAudioProps(prev => ({ ...prev, loading: false, error: message ?? "Ocorreu um erro ao reproduzir áudio" }))
  }
  ...
  function togglePlaylist() {
    setAudioControls(prev => ({ ...prev, playlistOpened: playlist ? !prev.playlistOpened : false }));
  }
    ...
    file && <div className={`fixed bottom-0 left-0 right-0 flex flex-col z-20 ${audioControls.playlistOpened ? 'top-0' : ''}`}>
      ...
      <div className="grid grid-cols-[calc(100%_-2rem)_2rem] bg-black bg-opacity-45 border-2 border-zinc-400 bg-gradient-to-r from-zinc-500/25 to-zinc-900/25 ps-4 backdrop-blur-sm animate-transform-from-bottom">
        <div className="w-full flex flex-col flex-grow">
          <div className="w-full flex flex-col md:flex-row my-2 md:my-4">
            <div className="w-full md:w-1/3 md:grid-cols-[3rem_calc(100%_-_3rem)] grid grid-cols-[3.5rem_calc(100%_-_3rem)]  gap-2 items-center mb-3 md:mb-0">
              <div className="flex flex-col justify-center items-center w-14 h-14 md:w-12 md:h-12 overflow-hidden rounded-md">
                <Image src={!file.icon ? "/img/icons/music.svg" : file.icon} alt={file.name} className={`h-full w-full top-0 left-0 object-cover ${audioControls.hideTitle ? 'blur-sm' : ''}`} width={512} height={512} unoptimized />
              </div>
              <div className="overflow-hidden md:text-left text-center">
                <h1 className={`font-bold overflow-hidden text-nowrap text-ellipsis ${audioControls.hideTitle ? 'blur-sm' : ''} ${audioProps.error ? 'text-red-400' : ''}`}>
            ...
            <div className="w-full flex md:w-2/3 md:justify-center">
              <div className="md:w-1/2 flex justify-center gap-3 me-auto md:me-0">
                ...
                <button className="text-3xl w-8 h-12" onClick={togglePlayAudio} disabled={audioProps.loading}>
                  {audioProps.loading ?
                    <div className="flex w-full h-full items-center justify-center">
                      <div className="w-6 h-6 border-4 border-b-transparent border-white rounded-full animate-spin"></div>
                    </div> :
                    <FontAwesomeIcon icon={audioProps.error ? faRotateRight : audioProps.playing ? faPause : faPlay} />
                  }
                </button>
              ...
              <div className="md:w-1/2 flex gap-2 justify-end">
                <button className="text-2xl flex flex-col justify-center" onClick={toggleRandom}>
                  {audioControls.random ?
                    <svg width="24px" height="32" viewBox="0 0 500 450" fill="white" xmlns="http://www.w3.org/2000/svg">
                ...
                <div className="flex items-center gap-2">
                  <button className="text-2xl w-7" onClick={toggleMute}>
                    <FontAwesomeIcon icon={audioControls.muted ? faVolumeMute : faVolumeHigh} />
                  </button>
                  <Range className="sm:block w-20 hidden" percent={audioControls.volume * 100} onChange={(volume) => {
                    setAudioControls(prev => ({ ...prev, volume: volume / 100 }));
                    return true;
                  }} />
                </div>
                <button className="text-2xl w-9">
                  <FontAwesomeIcon icon={audioControls.hideTitle ? faEye : faEyeSlash} onClick={toggleHideText} />
                </button>
          ...
          <Range className="w-full my-2" percent={audioTimePercent * 100} onChange={updateAudioPercent} />
          <audio
            autoPlay
            src={file.src}
            ref={audioRef}
